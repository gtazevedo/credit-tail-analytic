...
Uma vez que o estudo será disponibilizado juntamente dos códigos-fonte utilizados para sua elaboração (estarão disponíveis para 
consulta no no #link("https://github.com/gtazevedo/credit-tail-analytic")[repositório do GitHub] do autor, juntamente com a documentação de uso), 
todos os procedimentos detalhados abaixo podem ser replicados pelo leitor de forma independente. 
...
que pertence ao sistema REUNE. O período da amostra foi do segundo dia de janeiro de 2018 a 10 de julho de 2026, sendo que os dados diários extraídos possuem as seguintes colunas:
Codigo Cetip, Tipo, Agrupamento, Taxa Mínima, Taxa Média, Taxa Máxima, Preço Mínimo, Preço Médio, Preço Máximo e Faixa de Volume.
O leitor interessado em reproduzir esta coleta de dados pode consultar as rotinas de raspagem de dados (_web scraping_) e os _scripts_ de unificação (pois a raspagem de dados 
gera um arquivo csv por dia de consulta) disponibilizados no repositório público desta pesquisa no 
#link("https://github.com/gtazevedo/credit-tail-analytic")[GitHub].

Já as informações cadastrais foram obtidas na página #link("https://www.debentures.com.br")[Debentures.com.br] (que será substituída pelo #link("https://data.anbima.com.br/")[ANBIMA Data]) 
por meio de consulta automatiza utilizando o pacote `debentures_dot_com` (mantido pelo autor desse trabalho). As informações 
...
(parâmetros $alpha$ e $beta$) da variância condicional, sendo importante notar que esses ativos foram removidos apenas da amostra de teste, com a exceção de RDVT11, que foi removido manualmente da amostra
...
Os dados coletados em @subcap_coleta_dados foram divididos em dois períodos: _In-Sample_ e _Out-of-Sample_. O período _In-Sample_, iniciado em janeiro de 2018 até dezembro de 2022
 utilizado para a seleção e estimação dos parametros dos modelos e o período _Out-of-Sample_, iniciado em janeiro de 2023 até julho de 2026, utilizado para a implementação, observação e validação dos modelos,
...
foi aquele que apresentou menor valor de AIC, cada vez que o modelo foi eleito foi considerado uma vitória e, ao final, foi calculado a taxa média de vitórias de cada modelo e eleito aquele que apresentou um maior percentual de vitórias. 
...
Os resultados demonstram um excelente grau de aderência do modelo ao mercado de crédito secundário brasileiro, conforme ilustrado na @fig_diagnostico_residuos, 
onde 86,4% dos papéis modelados passaram no teste de Ljung-Box e 94,9% dos ativos passaram no teste ARCH-LM, o que evidencia que o modelo EGARCH é adequado para a maioria das debêntures analisadas.
...
na @tab_adf, demonstram que a série de _spread_ bruto (`Taxa_Ativo`) é não-estacionária em 91% dos casos, como esperado (a nível de _spread_, as séries são integradas de ordem 1) e após a primeira 
...
Os ativos que compõem os 3,4% restantes que não rejeitam H0 são, em sua maioria, papéis com histórico extremamente curto (entre 30 e 60 observações) e perfil de iliquidez estrutural, 
...
com $k=3$ regimes (a escolha dos regimes é justificada empiricamente na @subcap_kmeans_k3), a qualidade de separabilidade dos agrupamentos foi mensurada através das três métricas listadas na @sub_cap_clusters 
( *_Silhouette Score_* , *Índice Davies-Bouldin* e *Índice Calinski-Harabasz*).

Uma vez que as métricas atuam em ordens de grandeza e domínios matemáticos distintos, o subconjunto vencedor não é escolhido por médias, mas sim pelo método de agregação de Ranks de Borda (*Borda Count*)#footnote[A classificação integral de todas as combinações 
de variáveis testadas no _Grid Search_, contendo o ranking detalhado de cada métrica de particionamento e o Borda Score final, está disponível no repositório público da pesquisa, no diretório `anexos_digitais/`.]. 
O algoritmo computa a posição de cada combinação no ranking individual de cada métrica e o *Borda Score* final é dado pela soma das posições invertidas, garantindo uma eleição ordinal, determinística e 
...
incorporou também a estimativa de risco de cauda (`Expected_Shortfall_99`), formando o grupo `Taxa_ZScore`, `Volatilidade_EGARCH` e `Expected_Shortfall_99`, que figurou em segundo lugar no ranking geral, a @fig_feature_selection 
ilustra o Top 10 das combinações avaliadas. A escolha tática pelo trio de variáveis é justificada pela necessidade imperativa do modelo possuir sensibilidade preditiva a perdas financeiras extremas, 
...
Na @fig_feature_radar, o conjunto dado pelas variáveis `Taxa_ZScore`, `Volatilidade_EGARCH` e `Expected_Shortfall_99` (segundo lugar) e o conjunto dado pelas variáveis `Taxa_ZScore`, `Volatilidade_EGARCH` e `VaR_99` (terceiro lugar)
...
forma deliberada no desenho da pesquisa para servir como contraponto determinístico ao modelo markoviano (HMM), uma vez que a intenção é provar valor marginal preditivo que a memória temporal agrega sobre a classificação puramente estática, de forma empírica 
e quantitativa. O processo é realizado iterativamente para cada grupo de indexador (ex: DI, IPCA), a fim de respeitar as dinâmicas particulares de cada mercado.
...
com base na distância euclidiana inversa de cada observação _Out-of-Sample_ até o centróide Vermelho.
...
- *H1₂*: O modelo HMM, ao modelar a persistência dos regimes de risco, antecipa a deterioração do crédito com maior _lead time_ do que o K-Means para os eventos de crédito observados no período _Out-of-Sample_.
...
escolha do modelo Gaussiano para assegurar a estabilidade numérica e convergência na calibração das matrizes, da mesma forma, os vetores de médias estimadas para as emissões Gaussianas sofrem a correção 
heurística de _Label Switching_ para rotular os estados como Verde, Amarelo e Vermelho.

Uma alteração fundamental feita no HMM padrão diz respeito à calibração da Matriz de Transição de Estados ($A$). Em bases de dados com regimes muito duradouros, pode haver a total ausência empírica de transições entre estados extremos (como saltos diretos de 
Verde para Vermelho) na amostra de treinamento, o que gera probabilidades de transição iguais a zero, transformando os regimes em "estados absorventes" (uma vez que o modelo entre nesse estado, a probabilidade de sair matematicamente se 
anula). Para mitigar esse problema, aplicou-se primeiramente a *Suavização de Laplace* (_Additive Smoothing_) sobre a matriz empírica de contagens de transição:
...
buscando mesclar a inércia do particionamento geométrico (K-Means) com a memória temporal do filtro bayesiano (HMM).
...
estritamente sobre as predições do período _In-Sample_, na qual a métrica alvo para a otimização foi o *Calmar Ratio* (retorno anualizado sobre rebaixamento máximo) gerado pelo simulador financeiro. 
...
as saídas do motor EGARCH-t, utilizou-se o Teste de Proporção de Falhas (_POF - Proportion of Failures_) desenvolvido por 
...
pelo modelo, nesse caso, rejeitar a hipótese nula do teste indica que o motor de volatilidade subestima ou superestima
...
É simulado o impacto de diferentes estratégias de liquidação de portfólio baseadas nas pontuações de risco dos modelos
(Verde, Amarelo ou Vermelho), comparando duas abordagens de liquidação para cada modelo:

  - Venda Retardada: Estratégia reativa, onde a liquidação ocorre quando o modelo acusa um Regime de Risco Vermelho. Caso o modelo seja capaz de prever o estado vermelho antes do choque, esse modelo deveria gerar rentabilidade superior, uma vez que a venda ocorre antes do evento, evitando assim perdas substanciais. Todavia, caso isso não ocorra, essa estratégia tende a gerar perdas operacionais, uma vez que as perdas já estariam materializadas no preço dos ativos.
  - Venda Preventiva: Estratégia ofensiva, na qual a liquidação ocorre quando o modelo acusa um Regime de Risco Amarelo. Idealmente, deveria ser superada pela venda retardada no caso de um modelo ideal, porém, na prática, em cenários onde o modelo pode possuir lentidão na classificação  para o regime vermelho, essa estratégia tende a gerar rentabilidade superior, uma vez que a venda ocorre assim que são observados os primeiros sinais de deterioração do ativo, antes da materialização completa das perdas. Contudo, se houverem muitos falsos positivos, essa estratégia tende a gerar rentabilidade inferior à venda retardada, devido aos custos operacionais que reduzem o ganho em cenários de alta volatilidade.
...
custo de transação de 0,5% (50 _bps_). No mercado secundário de crédito brasileiro, caracterizado por menor liquidez e _spreads_ de _bid-ask_ mais elásticos do que o mercado de ações, a inclusão desse custo é fundamental, já que
...
O resultado das estratégias táticas é comparado contra o desempenho passivo de um portfólio _Buy-and-Hold_ e, para assegurar o rigor técnico e a reprodutibilidade da simulação financeira, 
...
- *Remuneração de Caixa:* Qualquer montante não alocado em debêntures (caixa livre) é remunerado diariamente pela taxa DI (CDI) histórica real correspondente ao dia da simulação, refletindo o custo de oportunidade livre de risco (_risk-free_). Buscando replicar a alocação padrão de um fundo de investimentos ou pessoa física, que manteriam esse dinheiro remunerado, por exemplo, em um fundo de zeragem ou CDB diário, respectivamente.
- *Regra de Recompra:* Para evitar re-entradas prematuras (_dead cat bounces_) e a corrosão da rentabilidade pelo excesso de giro, o capital em caixa é redistribuído igualitariamente apenas entre ativos que atendam simultaneamente aos três filtros:
